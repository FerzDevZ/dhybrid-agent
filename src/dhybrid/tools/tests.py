...
def run_tests(command: str = "pytest -q", path: str = ".", timeout: int = 120, max_chars: int = 8000) -> str:
    try:
        proc = subprocess.run(
            command, shell=True, cwd=path, capture_output=True, text=True, timeout=timeout, check=False
        )
...
def tdd_status(path: str = ".", timeout: int = 120) -> str:
    ...
    try:
        proc = subprocess.run(
            "pytest -q --no-header", shell=True, cwd=path, capture_output=True, text=True, timeout=timeout, check=False
        )
    except subprocess.TimeoutExpired:
