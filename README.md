...
- **Agent bisa MELIHAT & menerima paste apa pun (v0.7.0)** — `read_image` membacakan gambar/screenshot (vision LLM byNara; fallback OCR lokal tanpa API key via rapidocr-onnxruntime); `/shot` screenshot layar ke `~/.dhybrid/captures/`; `/paste` menyimpan teks tempel ke `~/.dhybrid/pastes/` dan langsung masuk konteks agent.
